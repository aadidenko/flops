	...
	expected := []VM{
		{
			...
			CurrentSnapshot:  nil,
			BackupPolicy:     VMBackupPolicy{1, 72},
...
func TestVM_Get(t *testing.T) {
	...
	mux.HandleFunc("/v1/vm/88534", func(w http.ResponseWriter, r *http.Request) {
		testMethod(t, r, "GET")
		fmt.Fprint(w, `{"status":"OK","result":{"id":88534,"name":"win server",
			"internalId":"vm88534","memory":1024,"disk":24576,"cpu":1,
			"bandwidth":100,"tariffId":20,"ipAddresses":[],
	...
	if !reflect.DeepEqual(vm, expected) {
		t.Errorf("VM.Get returned %+v, expected %+v", vm, expected)
	}
}
